...
function getDecisionDetails(res) {
    ...
    const isMember = hasDecision ? minK > threshold : res?.prediction?.tone === 'member';
    ...
    return {
        ...
        comparison: hasDecision ? `${minK.toFixed(4)} > ${threshold.toFixed(1)}` : 'N/A',
            ...
            <div class="flex flex-row flex-wrap gap-4">
                <div class="glass-panel px-5 py-3 rounded-xl border-white/5 flex flex-col justify-center min-w-[140px]">
                    <div class="text-lg sm:text-xl font-mono font-bold text-white">
                        ${formatNumber(m.ppl, 1)} <span class="text-xs text-slate-500 font-normal">/ ${formatNumber(m.loss_score, 2)}</span>
                    </div>
                ...
                <div class="glass-panel px-5 py-3 rounded-xl border-white/5 flex flex-col justify-center min-w-[140px]">
                    <div class="text-lg sm:text-xl font-mono font-bold text-white">${formatNumber(m.zlib_score, 4)}</div>
...
function renderResults() {
    ...
    STATE.resultsData.forEach((res, index) => {
        ...
        const badgeStyle = decision.tone === 'member'
        ...
        tr.innerHTML = `
            ...
            <td class="py-5 px-4 font-mono text-sm text-slate-400">${formatNumber(res.metrics.loss_score, 4)}</td>
            <td class="py-5 px-4 font-mono text-sm text-indigo-300">${formatNumber(res.metrics.zlib_score, 5)}</td>`;
