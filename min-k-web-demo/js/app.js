    ...
    try {
        ...
        if (failedResults.length > 0) {
            const failedNames = failedResults
                .map(r => `${r.model_name || r.model || 'model'}: ${r.error || 'failed'}`)
                .join(' | ');
    ...
    return `
        <div class="overflow-x-auto rounded-xl border border-white/10 bg-black/10">
            <table class="w-full text-sm text-left">
                ...
                <tbody>
                    ${rows.map(row => {
                        const isMinK = String(row.method || '').toLowerCase().includes('min-k');
                        return `
...
async function loadDashboard(force = false) {
    ...
    try {
        ...
        DOM.dashboardRocSlot.className = "relative z-10 border border-white/10 bg-black/10 rounded-xl p-4 flex items-center justify-center";
